...
<div id="sidebar" class="sidebar collapsed">
	...
	<ul class="sidebar-tabs" role="tablist" style="padding-top:10px;">
		...
		<li><a href="#edit" role="tab" id="edit"><i class="fa fa-edit" style="font-size:120%;"></i><span>Editace</span></a></li>
		<li><a href="#scenar" role="tab" id="scenar"><i class="fa fa-book" style="font-size:120%;"></i><span>Scénář</span></a></li>
		<li><a href="#navod" role="tab" id="navod2"><i class="fa fa-info" style="font-size:120%;"></i><span>Návod-ad.</span></a></li>
	...
	<div class="sidebar-content active">
		...
		<p>Kliknutím na název příslušné vrstvy se dostanete do editačního klienta. V něm je nutné být přihlášen (kliknout na login vpravo nahoře).<br /><br /></p>
			Vrstvy GEOJSON:<br /><?php
			$query50 = "SELECT * FROM vrstvy WHERE `typ` = 'GEOJSON' AND `zobrazeno` = 'ano' AND `".$adapt_idcko."` = 'ano' ORDER BY `item_order` ASC";
			$result50 = mysqli_query($con,$query50);
			if ( mysqli_num_rows($result50) ) {
			   while ($row50 = mysqli_fetch_assoc($result50)) {
					echo " <a class=\"upravy fancybox fancybox.iframe\" href=\"http://geojson.io/#id=gist:".$adapt_gist."/".$row50['odkaz']."&map=10/49.5/17.25\">".$row50['nazev']."</a><br /> ";
					/* echo " <a class=\"upravy\" target=\"_blank\" href=\"upravy.php?id=".$row50['odkaz']."&gist=".$adapt_gist."\">".$row50['nazev']."</a><br /> ";*/   
			   }
			   };			
			?>
			<br />
			Vrstvy Esri REST:<br /><?php
			$query50 = "SELECT * FROM vrstvy WHERE `typ` = 'ESRI' AND `zobrazeno` = 'ano' AND `".$adapt_idcko."` = 'ano' ORDER BY `item_order` ASC";
			$result50 = mysqli_query($con,$query50);
			if ( mysqli_num_rows($result50) ) {
			   while ($row50 = mysqli_fetch_assoc($result50)) {
					echo " <a class=\"upravy fancybox fancybox.iframe\" href=\"editacni/".substr($row50['odkaz'],-3, 1)."/\">".$row50['nazev']."</a><br /> ";
