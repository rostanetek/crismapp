...
<div id="sidebar" class="sidebar collapsed">
	...
	<ul class="sidebar-tabs" role="tablist" style="padding-top:10px;">
		...
		<li><a href="#navod" role="tab"><i class="fa fa-info" style="font-size:120%;"></i><span>Návod-ad.</span></a></li>
		...
		<p>Kliknutím na název příslušné vrstvy se dostanete do editačního klienta. V něm je nutné být přihlášen (kliknout na login vpravo nahoře).<br /><br /></p>
			...
			if ( mysqli_num_rows($result50) ) {
			   while ($row50 = mysqli_fetch_assoc($result50)) {
					echo " <a class=\"upravy fancybox fancybox.iframe\" href=\"http://geojson.io/#id=gist:".$gist."/".$row50['odkaz']."&map=10/49.5/17.25\">".$row50['nazev']."</a><br /> ";
			...
			$query50 = "SELECT * FROM vrstvy WHERE `typ` = 'ESRI' AND `zobrazeno` = 'ano' ORDER BY `item_order` ASC";
			$result50 = mysqli_query($con,$query50);
			if ( mysqli_num_rows($result50) ) {
			   while ($row50 = mysqli_fetch_assoc($result50)) {
...
echo "
var ikonka_".$row['id']." = L.MakiMarkers.icon({
icon: null,
color: \"".$row['color']."\"
});
";
...
$result2 = mysqli_query($con,$query2);
   while ($row = mysqli_fetch_assoc($result2)) {
		echo "	\"".$row['nazev']." ";
			...
			if ($row['typ_legenda']=="body" && $row['id']!="47") {echo "<span class='legend'><img src='https://api.tiles.mapbox.com/v3/marker/pin-m+".substr($row['color'], 1, 10).".png' class='ikonka_bodova'></span>\": vrstva_".$row['id'].", ";}
...
home: true,
  maxZoom: 21,
  center: [params.lat || 49.593687, params.lng || 17.251614], 
  zoom: params.zoom || 13,
  layers: layers,
  twoFingerZoom: true,
		contextmenu: true,
