...
class InternationalExample extends Common
{
	...
	protected function initiateEditor()
	{
		...
		$tableColumns['vrstva'] = array(
			'display_text' => 'ID vrstvy <br><span class="popisek">nutné pro spárování s konkrétní vrstvou</span>', 
			'perms' => 'EVCTAXQSHOF', 'req' => true
		);
		
		$tableColumns['hodnota'] = array(
			'display_text' => 'Hodnota atributu<br><span class="popisek">konkrétní hodnota atributu</span>', 
			'perms' => 'EVCTAXQSHOF', 'req' => true
		);
		
		$select_vetsimensi = array(
          '=' => '=',
          '>' => '>',
		  '<' => '<'
		); 
		...
		$tableColumns['vetsimensi'] = array(
			'display_text' => 'Operátor<br><span class="popisek">větší nebo rovná se</span>', 
			'perms' => 'EVCTAXQSHOF', 'req' => true,
			'select_array' => $select_vetsimensi,
			'default' => '=' 
		);
		
		$tableColumns['barva'] = array(
			'display_text' => 'Barva<br><span class="popisek">buď HEX (#F0F0F0) nebo název (red) <a href="http://www.w3schools.com/tags/ref_colorpicker.asp" target="_blank">vybrat</a></span>', 
			'perms' => 'EVCTAXQSHOF', 'req' => true
		);
